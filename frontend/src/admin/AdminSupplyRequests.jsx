// admin/AdminSupplyRequests.jsx
import { useState, useEffect } from 'react';
import { API_BASE } from '../constants/config';
import { Modal } from 'react-bootstrap';
import {
  Package, CheckCircle2, Ban, RefreshCw, Search, Filter,
  User, Calendar, Eye, CheckCircle, XCircle, Activity,
  Loader, Clock, AlertTriangle
} from 'lucide-react';

export function AdminSupplyRequests({ token }) {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/housekeepers/supply-requests/`, {
        headers: getHeaders()
      });
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    if (search) {
      filtered = filtered.filter(r => 
        ...
        r.housekeeper_name?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const updateStatus = async (id, newStatus, additionalData = {}) => {
    if (!window.confirm(`Are you sure you want to update this request to ${newStatus}?`)) return;
    
    setUpdating(true);
    setUpdatingId(id);
    
    try {
      const response = await fetch(`${API_BASE}/housekeepers/supply-requests/${id}/update/`, {
        method: 'PATCH',
        headers: getHeaders(),
  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ fontSize: '0.8rem', color: '#8a96a8' }}>Approve or reject supply requests from housekeepers</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', flex: 1 }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
            ...
            <input
              type="text"
              placeholder="Search by item, housekeeper, or room..."
              value={search}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '3rem', color: '#8a96a8' }}>No supply requests found</div>
      ) : (
              ...
              <div key={req.id} style={{ background: '#fff', border: `1px solid ${statusConfig.color}30`, borderRadius: 12, padding: '1rem', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    {req.status === 'APPROVED' && (
                      <button onClick={() => updateStatus(req.id, 'FULFILLED')} disabled={isUpdating} style={{ background: '#10b981', color: '#fff', border: 'none', borderRadius: 6, padding: '0.3rem 0.8rem', fontSize: '0.7rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.3rem', cursor: 'pointer' }}>
                ...
                <div style={{ fontWeight: 600, fontSize: '1rem', marginBottom: '0.25rem' }}>{req.item_name} x{req.quantity}</div>
                <div style={{ fontSize: '0.75rem', color: '#4a5568', marginBottom: '0.5rem' }}>{req.reason}</div>
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', fontSize: '0.7rem', color: '#8a96a8' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}><User size={11} /> {req.housekeeper_name}</span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>Room {req.room_number}</span>
      ...
      <Modal show={showDetail} onHide={() => { setShowDetail(false); setSelectedRequest(null); }} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title>Supply Request Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedRequest && (
            <>
              <div><strong>Item:</strong> {selectedRequest.item_name}</div>
              <div><strong>Quantity:</strong> {selectedRequest.quantity} {selectedRequest.unit}</div>
              <div><strong>Reason:</strong> {selectedRequest.reason}</div>
              <div><strong>Housekeeper:</strong> {selectedRequest.housekeeper_name}</div>
              <div><strong>Room:</strong> {selectedRequest.room_number}</div>
