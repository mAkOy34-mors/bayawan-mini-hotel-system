...
const css = `
  ...
  .asb-mark{width:30px;height:30px;border-radius:9px;background:linear-gradient(135deg,#9a7a2e,#C9A84C);display:flex;align-items:center;justify-content:center;font-size:.78rem;color:#fff;flex-shrink:0;box-shadow:0 2px 8px rgba(201,168,76,0.3)}
  ...
  .asb-ico{width:28px;height:28px;border-radius:7px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#f1f5f9;font-size:.88rem;border:1px solid transparent;transition:background .16s}
  .asb-btn.on .asb-ico{background:rgba(201,168,76,0.15);border-color:rgba(201,168,76,0.2)}
  ...
  .asb-out-ico{width:28px;height:28px;border-radius:7px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:rgba(220,53,69,0.08);font-size:.88rem}
...
const NAV = [
  { key:'dashboard',       label:'Dashboard',        icon:'📊', section:'Management' },
  { key:'bookings',        label:'Bookings',          icon:'🛏️' },
  { key:'change-requests', label:'Change Requests',   icon:'🔄', badge:'pendingChanges' },
  { key:'rooms',           label:'Rooms',             icon:'🏨' },
  { key:'guests',          label:'Guests',            icon:'👥' },
  { key:'payments',        label:'Payments',          icon:'💳' },
  { key:'rewards',         label:'Rewards',           icon:'⭐', section:'Operations' },
  { key:'support',         label:'Support',           icon:'💬', badge:'pendingTickets' },
  ...
  return <>
    ...
    <div className="asb-logo">
      <div className="asb-logo-row">
        <div className="asb-mark">✦</div>
        <div>
          <div className="asb-name">Cebu Grand Hotel</div>
          <div className="asb-sub">Admin Panel</div>
        </div>
      </div>
      <div className="asb-badge">⚙ Administrator</div>
    </div>
    ...
    <nav className="asb-nav">
      {NAV.map(({ key, label, icon, section, badge }) => (
        <div key={key}>
          ...
          <button className={`asb-btn${page===key?' on':''}`} onClick={()=>setPage(key)}>
            <span className="asb-ico">{icon}</span>
            {label}
      ...
      <button className="asb-out" onClick={onLogout}>
        <span className="asb-out-ico">🚪</span>Sign Out
      </button>
    </nav>
    <div className="asb-foot">© 2026 Cebu Grand Hotel</div>
  </>;
}
    ...
    <Offcanvas show={open} onHide={onClose} placement="start" className="asb-oc" style={{width:252}}>
      <Offcanvas.Header closeButton>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.05rem',fontWeight:600,color:'#1a1f2e',display:'flex',alignItems:'center',gap:'.45rem'}}>
          <div style={{width:24,height:24,borderRadius:7,background:'linear-gradient(135deg,#9a7a2e,#C9A84C)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.7rem',color:'#fff'}}>✦</div>
          Admin Panel
        </div>
      </Offcanvas.Header>
      <Offcanvas.Body>
