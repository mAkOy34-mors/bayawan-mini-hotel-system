...
export function AdminPayments({ token }) {
  ...
  return (
    <div className="ap-root">
      ...
      <div className="ap-hd">
        <div><h1 className="ap-title">Payments</h1><p className="ap-sub">Monitor and verify all transactions</p></div>
      </div>
      ...
      {!loading && pending.length > 0 && (
        <div style={{ display:'flex', alignItems:'center', gap:'.85rem', padding:'.9rem 1.15rem', borderRadius:12, marginBottom:'1.25rem', background:'rgba(245,158,11,0.07)', border:'1px solid rgba(245,158,11,0.25)', animation:'fadeUp .4s both' }}>
          ...
          <div style={{ flex:1, fontSize:'.83rem', color:'#92400e', fontWeight:600 }}>{pending.length} payment{pending.length>1?'s':''} awaiting manual verification</div>
          <button className="ap-btn-ghost" style={{ fontSize:'.76rem', padding:'.3rem .75rem' }} onClick={()=>setFilter('PENDING')}>View Pending →</button>
        </div>
      )}
      ...
      <div className="ap-panel">
          ...
          <div style={{ display:'flex', gap:'.4rem', flexWrap:'wrap' }}>
            {STATUS_OPTS.map(s => (
              <button key={s||'all'} onClick={()=>{setFilter(s);}}
                style={{ padding:'.32rem .78rem', borderRadius:99, fontFamily:"'DM Sans',sans-serif", fontSize:'.72rem', fontWeight:600, cursor:'pointer', transition:'all .15s',
        ...
        {loading
          ? <div style={{ padding:'2.5rem', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:'.65rem' }}><Spinner/><span style={{ color:'var(--text-muted)', fontSize:'.78rem' }}>Loading payments…</span></div>
          : payments.length === 0
            ? <div className="ap-empty"><div className="ap-empty-ico">💳</div><div className="ap-empty-title">No payments found</div></div>
            : <>
                <div style={{ overflowX:'auto' }}>
                  <table className="ap-tbl">
                    <thead>
                      <tr><th>ID</th><th>Description</th><th>Email</th><th>Type</th><th>Amount</th><th>Status</th><th>Date</th><th>Paid At</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                      {visible.map(p => (
                        <tr key={p.id}>
                          ...
                          <td style={{ maxWidth:180 }}>
                            <div style={{ fontWeight:600, color:'var(--text)', fontSize:'.82rem', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.description||'—'}</div>
                          ...
                          <td>
                            <div style={{ display:'flex', alignItems:'center', gap:'.35rem' }}>
                              ...
                              <span style={{ fontSize:'.75rem', color:'var(--text-sub)' }}>{p.type?.replace('_',' ')||'—'}</span>
                            </div>
                          </td>
                          <td><span style={{ fontWeight:700, color:'var(--text)', whiteSpace:'nowrap' }}>{fmt(p.amount)}</span></td>
                          <td><Pill status={p.status}/></td>
                          <td style={{ fontSize:'.74rem', color:'var(--text-muted)', whiteSpace:'nowrap' }}>{fmtDate(p.createdAt)}</td>
                          <td style={{ fontSize:'.74rem', color: p.paidAt ? 'var(--green)' : 'var(--text-muted)', whiteSpace:'nowrap' }}>{p.paidAt ? fmtDate(p.paidAt) : '—'}</td>
                          <td>
                            <div style={{ display:'flex', gap:'.35rem' }}>
                              <button className="ap-btn-ghost" style={{ fontSize:'.72rem', padding:'.28rem .6rem' }} onClick={()=>openDetail(p.id)}>View</button>
                              {p.status === 'PENDING' && (
                                <button className="ap-btn-green" style={{ fontSize:'.72rem', padding:'.28rem .6rem' }} onClick={()=>verify(p.id)}>✓ Verify</button>
                              )}
                            </div>
                          </td>
                        </tr>
      ...
      <Modal show={showDetail} onHide={()=>setShowDetail(false)} size="md" centered className="ap-modal">
        <Modal.Header closeButton><Modal.Title>Payment Detail</Modal.Title></Modal.Header>
        <Modal.Body>
          {detailLoading
            ...
            : <>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'.9rem 1rem', background:'var(--surface2)', borderRadius:10, border:'1px solid var(--border)', marginBottom:'1rem' }}>
                  <div>
                    ...
                    <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'1.85rem', fontWeight:600, color:'var(--text)' }}>{fmt(detail.amount)}</div>
                  </div>
