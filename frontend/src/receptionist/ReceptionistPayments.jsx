...
export function ReceptionistPayments({ token }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      show('Failed to load payments', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const netCollected = totalPaidAmount - totalRefundedAmount;
  ...
  const searchBooking = async () => {
    ...
    try {
      ...
      if (match) {
        setCashBook(match);
        setCashAmt(String(parseFloat(match.remainingAmount || 0)));
      } else {
  ...
  const recordCash = async () => {
    if (!cashBook) {
      show('Find a booking first', 'error');
      return;
    }
    if (!cashAmt || parseFloat(cashAmt) <= 0) {
    ...
    try {
      ...
      show(`Cash payment of ${fmt(cashAmt)} recorded!`, 'success');
      setShowCash(false);
      setBookRef('');
      setCashAmt('');
      setCashNote('');
  ...
  return (
    <div className="ap-root">
      ...
      <div className="ap-stats">
        {[
          { label: 'Net Collected', value: fmt(netCollected), color: 'green' },
          { label: 'Pending', value: fmt(totalPending), color: 'orange' },
          { label: 'Total Refunds', value: fmt(totalRefunds), color: 'red' },
      ...
      <div className="ap-panel">
        <div className="ap-panel-hd">
          <div>
            <div className="ap-panel-title">Transactions</div>
            <div className="ap-panel-sub">{!loading && `${filtered.length} records`}</div>
          </div>
        ...
        {loading ? (
          ...
          <div className="ap-empty">
            <div style={{ display: 'flex', justifyContent: 'center', opacity: .25, marginBottom: '.65rem' }}>
                  ...
                  {visible.map(p => {
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td>
                          <span style={{ fontWeight: 700, fontSize: '.82rem', color: refund ? 'var(--red)' : 'var(--text)', whiteSpace: 'nowrap' }}>
                            {refund ? '−' : ''}{fmt(p.amount)}
                          </span>
                        </td>
                        <td style={{ fontSize: '.75rem', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{fmtDate(p.createdAt || p.paidAt)}</td>
                        <td>
                          <span className={`ap-pill ${refund ? 'REFUNDED' : p.status}`} style={refund ? { background: 'var(--red-bg)', color: 'var(--red)', borderColor: 'rgba(220,53,69,0.25)' } : {}}>
      ...
      <Modal show={showCash} onHide={() => setShowCash(false)} centered className="ap-modal">
        ...
        <Modal.Footer>
          ...
          <button
            ...
            disabled={recording || !cashBook || !cashAmt}
