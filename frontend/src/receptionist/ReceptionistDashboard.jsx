// ReceptionistDashboard.jsx - Simplified (no emergency panel, handled globally)
  ...
  const load = async () => {
    ...
    try {
      const [bookRes, roomRes] = await Promise.all([
        fetch(`${BASE}/receptionist/bookings/`, { headers: h(token) }),
      ...
      const arrivals = bookingsArray.filter(b => 
        b.checkInDate?.slice(0, 10) === today && 
        (b.status === 'CONFIRMED' || b.status === 'PENDING_DEPOSIT')
      );
      
      const departures = bookingsArray.filter(b => 
        b.checkOutDate?.slice(0, 10) === today && 
        b.status === 'CHECKED_IN'
      );
      
      const checkedIn = bookingsArray.filter(b => b.status === 'CHECKED_IN').length;
      ...
      console.log("Dashboard data:", { arrivals: arrivals.length, departures: departures.length, checkedIn });
        ...
        <div className="ap-panel" style={{ marginBottom: 0 }}>
          ...
          {loading ? (
            ...
            <div style={{ overflowX: 'auto' }}>
              <table className="ap-tbl">
                <thead>
                  <tr><th>Ref</th><th>Guest</th><th>Room</th><th>Remaining</th></tr>
                </thead>
                <tbody>
      ...
      <div className="ap-panel">
        ...
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="ap-tbl">
              <thead>
                <tr><th>Reference</th><th>Guest</th><th>Room</th><th>Check-In</th><th>Check-Out</th><th>Status</th></tr>
              </thead>
              <tbody>
